...
def horizontal_symmetry(region):
    ...
    top = img[:mid, :]
    bottom = img[h - mid:, :]
    t_sum, b_sum = np.sum(top), np.sum(bottom)
    return t_sum / b_sum if b_sum > 0 else 0
...
def extractor(region):
    ...
    h_sym = horizontal_symmetry(region)
    return np.array([region.area/region.image.size, cx, cy, perimeter, holes, vlines, hlines, eccentricity, aspect, v_sym, h_sym])
